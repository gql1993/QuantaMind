...
def resonator_layout_v1(coupling_length_list, resonator_length_list, resonator_num=5, chip_size=6,version='1.0'):
    ...
    design.delete_all_components()
    coupling_length_list = coupling_length_list
    resonator_length_list = resonator_length_list
    chip_size = chip_size
    cut_size = 0.5
    pad_width = 0.245
    pad_height = 0.245
    ...
    if version != '0.00':
        options = {
            ...
            'pos_y': str(-0.9*pos_x)+'mm',
            'spacing': '30um',
        }
        NumberComponent(design, name='number1', options=options)

    resonator_num = resonator_num
    shift = 0.
    rr_space = 2 * (pos_x - 0.535) / (resonator_num - 1)
    ql_distance = 1.65
    qq_space = 1
    # qpos_x = 0
    # qpos_y = -1.65
    coupling_length = coupling_length_list
    tee_name = 'TQ'
    ...
    for i in range(1, resonator_num + 1):
        ...
        MyCoupledLineTee(design,
                       tee_name + str(i),
                       options=Dict(pos_x=(-(resonator_num-1)/2 -1 + i) * rr_space - shift, pos_y='0mm',
                                    orientation=orientation,
                                    coupling_length=str(coupling_length[i - 1]) + 'um',
                                    coupling_space='3um',
                                    prime_width='10um',
                                    prime_gap='5um',
                                    second_width='10um',
    ...
    # draw tee_line connection
    if design.components[tee_name + str(1)].parse_options().orientation == 0:
        pin = list(design.components[tee_name + str(1)].pins.keys())[0]
    else:
        pin = list(design.components[tee_name + str(1)].pins.keys())[1]
    ops_ol = Dict(hfss_wire_bonds=False,
                  pin_inputs=Dict(start_pin=Dict(component=launch_pad_name + str(1),
                                                 pin='tie'),
                                  end_pin=Dict(component=tee_name + str(1), pin=pin)), trace_width='10um',
                  trace_gap='5um')
    RouteStraight(design, 'cpw_openLeft', options=ops_ol)

    for i in range(1, resonator_num):
        if design.components[tee_name + str(i)].parse_options().orientation == 0:
            start_pin = list(design.components[tee_name + str(i)].pins.keys())[1]
        else:
            start_pin = list(design.components[tee_name + str(i)].pins.keys())[0]
        if design.components[tee_name + str(i + 1)].parse_options().orientation == 0:
            end_pin = list(design.components[tee_name + str(i + 1)].pins.keys())[0]
        else:
            end_pin = list(design.components[tee_name + str(i + 1)].pins.keys())[1]
        ops = Dict(hfss_wire_bonds=False,
                   pin_inputs=Dict(start_pin=Dict(component=tee_name + str(i), pin=start_pin),
                                   end_pin=Dict(component=tee_name + str(i + 1), pin=end_pin)), trace_width='10um',
                   trace_gap='5um')
        RouteStraight(design, 'cpw' + str(i), options=ops)

    if design.components[tee_name + str(resonator_num)].parse_options().orientation == 0:
        pin = list(design.components[tee_name + str(1)].pins.keys())[1]
    else:
        pin = list(design.components[tee_name + str(1)].pins.keys())[0]
    ops_or = Dict(hfss_wire_bonds=False,
                  pin_inputs=Dict(start_pin=Dict(component=tee_name + str(resonator_num),
                                                 pin=pin),
                                  end_pin=Dict(component=launch_pad_name + str(2), pin='tie')), trace_width='10um',
                  trace_gap='5um')
    RouteStraight(design, 'cpw_openRight', options=ops_or)
    # gui.rebuild()
    # gui.autoscale()
    # draw spare resonator for Q factor test purpose
    # shift_r is the distance between x position of beginning vertical part of resonator and last vertical part of resonator
    ...
    for i in range(1, resonator_num + 1):
        ...
        if design.components[tee_name + str(i)].parse_options().orientation == 180:
            OpenToGround(design, open_to_ground_name + str(i),
                         options=Dict(
                             pos_x=design.components[tee_name + str(i)].get_pin('second_end').middle[0] + shift_r,
                             pos_y=TQ_y, width='10um', gap='5um', orientation='90', ))
            options = Dict(total_length=str(total_length[i - 1]),
                           hfss_wire_bonds=False,
                           pin_inputs=Dict(end_pin=Dict(component=tee_name + str(i),
                                                        pin='second_end'),
...
def twenty_qubits_tunable_coupler_layout_v1(parameter_file_path=None,jj_dict=None,airbridege=False,version='1.00'):
    ...
    design = designs.DesignPlanar()
    design.chips['main']['material'] = 'sapphire'
    design.chips['main']['size']['sample_holder_top'] = '430um'
    design.chips['main']['size']['sample_holder_bottom'] = '430um'
    design.chips['main']['size']['size_z'] = '-430um'
    # design.overwrite_enabled = True
    ...
    if airbridege is True:
        ...
        parameter_dict = Dict(pad_width='24 um', etch_residue='-3 um',
                              bridge_length='52 um', pad_layer=3, etch_layer=4)
...
def build_airbridge(design, list_meander, list_controlLine=None, parameter_dict=None, edge_space=0.15,
    ...
    if parameter_dict is None:
        parameter_dict = Dict(pad_width='24 um', etch_residue='4 um',
                              bridge_length='70um', pad_layer=3, etch_layer=4)
