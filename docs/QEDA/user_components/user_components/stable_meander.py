...
class StableRouteMeander(QRoute):
    ...
    Default Options:
        * meander: Dict
            ...
            * asymmetry='0um' --automatic asymmetry depends on the total length.
        * snap: 'true'
    ...
    def make(self):
        ...
        # print(f'start_point,end_point are {meander_start_point.position,meander_end_point.position}')
    ...
    def connect_meandered(self, start_pt: QRoutePoint,
        ...
        # print(f'forward,sideways are {forward,sideways}')

        # Calculate lengths and meander number
        dist = end_pt.position - start_pt.position
        # print(end_pt,start_pt.position)
        if snap:
        ...
        if meander_number % 2 == 0:
            sign_shift = -1.0
        else:
            sign_shift = 1.0

        shift_sideways = sign_shift * np.cross(sideways, forward) * mao.dot(dist, sideways)
        shift_forward = mao.dot(dist, forward)
        # print(f'shift_sideways,shift_forward are {shift_sideways, shift_forward}')

        res_length_forward = (shift_forward - self.p.fillet) - (meander_number-1) * self.p.meander.spacing
        # print(f'res_length_forward are {res_length_forward, self.p.meander.spacing}')
        length_x = (self.p.total_length - self.head.length - self.tail.length + shift_sideways + 2 * self.p.fillet
                    - (meander_number * math.pi * self.p.fillet + res_length_forward)) / (meander_number - 1)
        # print(f'length_x, meander_number are {length_x,meander_number}')
        if length_x / 2 - shift_sideways <= self.p.fillet or length_x / 2 <= self.p.fillet:
            asymmetry = -np.sign(shift_sideways*sign_shift)*2*self.p.fillet
        else:
            asymmetry = 0
        # print(asymmetry)
        if meander_number < 1:
        ...
        if start_meander_direction > 0:  # sideway direction
            ...
            if end_meander_direction > 0:  # sideway direction
                ...
                first_meander_sideways = True
        # print(start_meander_direction, end_meander_direction,first_meander_sideways)
