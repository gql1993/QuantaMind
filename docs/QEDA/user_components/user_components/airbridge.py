...
from qiskit_metal.qlibrary.user_components.my_qcomponent import Airbridge
...
class AirbridgeGenerator:
    ...
    * target_cpws: (list of QComponent) -- which CPW do you want to generate airbridges on?
        * TODO: if target_cpw == None, default to all CPWs
    * crossover_length: (float, in units mm) -- The length of the bridge (Defaults to 27um)
    * mode: 1 for keeping both straight and turn airbridges, 2 for only keeping straight airbridges,
            3 for only keep alternate straight airbridges

    Parameters associated w/ MIT LL Design Guidelines V1
    * BRS1: (float, in units mm) -- Bridge Minimum Spacing (Defaults to MIT LL standard, >= 5um)
    * BRS2: (float, in units mm) -- Bridge Pitch, spacing between the centers of each bridge (Defaults to MIT LL standard, =70um)

    '''

    def __init__(self, design, target_cpws=[], mode=1, BRS1=0.03, BRS2=0.070,options=Dict(pad_width='10 um', etch_residue='2 um', bridge_length='33 um')):
        '''
        ...
        if (len(target_cpws) == 0):  # If we get default target_cpws, get all CPWs in design
            pass
        else:  # Use the specified CPWs in the given list
            for target_cpw in self.target_cpws:
                coordinates = self.find_ab_placement(target_cpw,mode=self.mode)
                for coord in coordinates:
                    try:
                        name = target_cpw.name+"_AB{}".format(counter)
                        airbridge_qcomponent = self.make_single_ab(coord, name, self.options)
                        self.airbridges.append(airbridge_qcomponent)
                        counter += 1
                    except:
                        pass

    def make_single_ab(self, coord, name, ab_options):
        '''
        ...
        * name (string) -- Name this airbridge.
        ...
        y = coord[1]
        orientation = coord[2]
        options = Dict(pos_x=x, pos_y=y, angle=orientation, **ab_options)
    ...
    def find_ab_placement(self, target_cpw, precision=12, mode=1):
        '''
        ...
        * precision: (int) -- How precise did you define your CPWs?
        ...
        for i in range(len(points) - 1):
            ...
            if fillet > self.BRS1:
                ...
                lprime = mag_dl - 2 * self.BRS1
            n = 0  # refers to the number of bridges you've already placed
            # Asking should I place another? If true place another one.
            while (lprime) >= ((n+1) * self.BRS2):
                n += 1

            mu_x = (xf + x0) / 2
            mu_y = (yf + y0) / 2
            if mode == 2:
                ab_placements.append((mu_x, mu_y, np.degrees(theta)))
            if n!=0:
                ...
                if mode==1 or mode==3:
                    ...
                    # N = int(0.5*lprime/self.BRS2)
                    # print('N={}'.format(N))
                    # for i in range(N):
                    #     ab_placements.append((mu_x-i*self.BRS2*np.cos(theta), mu_y-i*self.BRS2*np.sin(theta), np.degrees(theta)))
                    #     ab_placements.append((mu_x + i * self.BRS2 * np.cos(theta), mu_y + i * self.BRS2 * np.sin(theta), np.degrees(theta)))                  # This is for the corner points
                    points_theta.append(theta)

            else:
                points_theta.append(theta)
        if mode==2:
            ab_placements =ab_placements[:1]+[t for t in ab_placements[1:-1] if t[-1] == ab_placements[-4][-1]]+ab_placements[-1:]
        # elif mode==3:
        #     temp = [t for t in ab_placements[1:-1] if t[-1] == ab_placements[-2][-1]]
        #     ab_placements = ab_placements[:1] + temp[::2] + ab_placements[-1:]
        elif mode==1:
